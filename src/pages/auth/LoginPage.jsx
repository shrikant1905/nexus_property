...
import { Mail, Lock, ArrowRight, Shield, Eye, EyeOff, Wrench, AlertCircle, Loader2, Building2 } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (user.roleKey === 'office-team') {
  ...
  const handleQuickLogin = async (role) => {
    ...
    try {
      ...
      if (role === 'ADMIN') {
        qEmail = import.meta.env.VITE_QUICK_LOGIN_ADMIN_EMAIL;
        qPass = import.meta.env.VITE_QUICK_LOGIN_ADMIN_PASS;
      } else if (role === 'OFFICE_TEAM') {
        qEmail = import.meta.env.VITE_QUICK_LOGIN_OFFICE_EMAIL;
        qPass = import.meta.env.VITE_QUICK_LOGIN_OFFICE_PASS;
      } else if (role === 'MAINTENANCE_STAFF') {
        qEmail = import.meta.env.VITE_QUICK_LOGIN_TECH_EMAIL;
        qPass = import.meta.env.VITE_QUICK_LOGIN_TECH_PASS;
      }

      if (!qEmail || !qPass) {
        throw new Error('Quick login credentials not configured in environment.');
      }

      const user = await login(qEmail, qPass);
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-between p-4 relative overflow-hidden select-none">
      ...
      <div className="flex-1 flex items-center justify-center w-full z-10 py-8">
        ...
        >
          ...
          {/* Quick Access Login */}
          {import.meta.env.VITE_QUICK_LOGIN_ENABLED === 'true' && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <div className="space-y-2">
                <p className="text-[11px] font-bold text-slate-500 text-center mb-3">Quick Access Login (Dev Mode)</p>
                <div className="flex flex-col gap-2.5">
                  <button
                    type="button"
                    onClick={() => handleQuickLogin('ADMIN')}
                    disabled={loading}
                    className="flex items-center justify-center gap-2 bg-[#00204a]/5 hover:bg-[#00204a]/10 border border-[#00204a]/15 rounded-xl px-4 py-3 transition-all cursor-pointer text-[#00204a] font-extrabold text-sm disabled:opacity-50"
                  >
                    <Shield size={18} />
                    👑 Admin
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => handleQuickLogin('OFFICE_TEAM')}
                    disabled={loading}
                    className="flex items-center justify-center gap-2 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl px-4 py-3 transition-all cursor-pointer text-blue-800 font-extrabold text-sm disabled:opacity-50"
                  >
                    <Building2 size={18} />
                    🏢 Office Team
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => handleQuickLogin('MAINTENANCE_STAFF')}
                    disabled={loading}
                    className="flex items-center justify-center gap-2 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-xl px-4 py-3 transition-all cursor-pointer text-emerald-800 font-extrabold text-sm disabled:opacity-50"
                  >
                    <Wrench size={18} />
