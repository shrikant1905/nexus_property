...
export default function MaintenanceDashboardPage() {
  ...
  const loadSupportingData = useCallback(async () => {
    try {
      const [jobsRes, staffRes, tenantsRes, inventoryRes] = await Promise.all([
        jobService.getJobs().catch(() => ({ data: [] })),
        staffService.getStaff().catch(() => ({ data: [] })),
        tenantService.getTenants().catch(() => ({ data: [] })),
        user?.roleKey === 'office-admin' ? inventoryService.getInventory({ lowStock: true }).catch(() => ({ data: [] })) : Promise.resolve({ data: [] }),
      ]);
      ...
      setLowStockItems(Array.isArray(inventoryRes) ? inventoryRes : inventoryRes?.data || []);
    } catch (_) {
  ...
  const handleRestockSubmit = async (e) => {
    ...
    // Explicitly reject decimals or negative values
    if (!/^\d+$/.test(restockQuantity) || parseInt(restockQuantity, 10) <= 0) {
    ...
    try {
      await inventoryService.restockItem(selectedRestockItem.id, parseInt(restockQuantity, 10), 'Restocked from dashboard');
      showToast('✓ Item restocked successfully!');
      setRestockModalOpen(false);
      setRestockQuantity('');
      setSelectedRestockItem(null);
      loadSupportingData(); // Refresh low stock items
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden text-slate-800">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="bg-white border border-rose-200 rounded-2xl p-5 sm:p-6 shadow-sm"
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-extrabold text-[#00204a] flex items-center gap-2">
            <AlertTriangle size={18} className="text-rose-600" /> Warehouse Low Stock Alerts
          </h3>
          <button
            onClick={() => navigate('/admin/inventory')}
            className="text-xs font-bold text-[#00204a] hover:text-rose-700 flex items-center gap-1 transition-colors"
          >
        ...
        {lowStockItems.length === 0 ? (
          <div className="text-sm text-slate-500 py-4 text-center bg-slate-50 rounded-xl border border-slate-200">
            ✓ All inventory items are adequately stocked. No low-stock alerts.
          </div>
        ) : (
          <div className="space-y-3">
            {lowStockItems.map(item => (
              <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-rose-50 border border-rose-200 gap-3">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white border border-rose-200 text-rose-600 flex items-center justify-center flex-shrink-0 font-bold">
                    <Boxes size={20} />
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-900">{item.itemName}</h4>
                    <p className="text-xs text-slate-600 mt-0.5">
                      Current: <span className="font-bold text-rose-600">{item.currentQuantity} {item.unit}</span> | Min: {item.minThreshold} {item.unit}
                    </p>
                    <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-rose-100 text-rose-800 border border-rose-200">
                      {item.stockStatus}
                    </span>
                  </div>
                </div>
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 transition-colors shadow-sm self-start sm:self-auto"
                >
                  1-Click Restock
                </button>
              </div>
      ...
      <FormModal
        ...
        title={`Restock: ${selectedRestockItem?.itemName}`}
        onSubmit={handleRestockSubmit}
        submitLabel="Confirm Restock"
      >
        <div className="p-2 space-y-4">
          <div className="bg-rose-50 p-3 rounded-xl border border-rose-100">
            <p className="text-xs text-slate-700">Current Quantity: <strong>{selectedRestockItem?.currentQuantity} {selectedRestockItem?.unit}</strong></p>
            <p className="text-xs text-slate-700">Minimum Threshold: <strong>{selectedRestockItem?.minThreshold} {selectedRestockItem?.unit}</strong></p>
          </div>
          <FormField
