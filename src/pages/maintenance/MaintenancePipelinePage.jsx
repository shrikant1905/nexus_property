...
export default function MaintenancePipelinePage() {
  ...
  // Load smart suggestion when a job is selected
  useEffect(() => {
    if (selectedJob && selectedJob.address) {
      setLoadingSuggestion(true);
      mapService.getSmartSuggestion(selectedJob.address, staffList)
                    ...
                    sectionJobs.map(job => (
                      ...
                      >
                        ...
                        {job.priority && job.priority !== 'NORMAL' && (
                          <div className={`text-[10px] font-bold px-2 py-0.5 rounded-lg w-fit ${
                            job.priority === 'URGENT' ? 'bg-red-50 text-red-700 border border-red-200' :
                        ...
                        {(job.assignedStaffName || (job.assignedStaffIds && job.assignedStaffIds.length > 0)) && (
                          <div className="flex -space-x-2 pt-1">
                            {(job.assignedStaffNames || [job.assignedStaffName]).filter(Boolean).map((name, i) => (
                              <div key={i} title={name} className="w-6 h-6 rounded-full bg-[#00204a] border-2 border-white flex items-center justify-center text-[9px] font-bold text-white shadow-sm z-10">
                                {name.charAt(0)}
                              </div>
                            ))}
                          </div>
                        ...
                        sectionJobs.map(job => (
                          ...
                          >
                            ...
                            {(job.assignedStaffName || job.scheduledDate) && (
                              <div className="space-y-1 pt-0.5 flex flex-col items-start gap-1">
                                {job.priority && job.priority !== 'NORMAL' && (
                                  <div className={`text-[9px] font-bold px-1.5 py-0.5 rounded border w-fit ${
                                ...
                                {(job.assignedStaffName || (job.assignedStaffIds && job.assignedStaffIds.length > 0)) && (
                                  <div className="flex -space-x-1.5">
                                    {(job.assignedStaffNames || [job.assignedStaffName]).filter(Boolean).map((name, i) => (
                                      <div key={i} title={name} className="w-5 h-5 rounded-full bg-[#00204a] border border-white flex items-center justify-center text-[8px] font-bold text-white shadow-sm z-10">
                                        {name.charAt(0)}
                                      </div>
                                    ))}
                                  </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-slate-800">Assigned Technicians (Multi-Select)</label>
            <div className="grid grid-cols-2 gap-2 bg-slate-50 border border-slate-200 rounded-xl p-3 max-h-32 overflow-y-auto">
              {staffList.map(s => {
                const staffId = s.profileId || s.id;
                const isSelected = form.assignedStaffIds.includes(staffId);
                return (
                  <label key={staffId} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={(e) => {
                      ...
                      className="w-3.5 h-3.5 text-[#00204a] rounded border-slate-300 focus:ring-[#00204a]"
                    />
          ...
          <div className="space-y-4 text-xs">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-2">
                📅 Allocate Job to Staff Calendar (Multi-Select):
              </label>
              <div className="grid grid-cols-2 gap-2 bg-[#111827] border border-white/15 rounded-xl p-3 max-h-32 overflow-y-auto">
                {staffList.map(s => {
                  const staffId = s.profileId || s.id;
                  const isSelected = selectedJob.assignedStaffIds?.includes(staffId) || String(selectedJob.assignedStaffId) === String(staffId);
                  return (
                    <label key={staffId} className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        onChange={async (e) => {
                          const checked = e.target.checked;
                          let newStaffIds = selectedJob.assignedStaffIds || (selectedJob.assignedStaffId ? [selectedJob.assignedStaffId] : []);
                          if (checked) {
                            newStaffIds = [...newStaffIds, staffId];
                          } else {
                            newStaffIds = newStaffIds.filter(id => String(id) !== String(staffId));
                          }
                          
                          try {
                            await jobService.updateJobStatus(selectedJob.id, { assignedStaffIds: newStaffIds });
                            await loadPipelineData();
                            
                            const updatedNames = staffList.filter(st => newStaffIds.includes(st.profileId || st.id)).map(st => st.name);
                            setSelectedJob(prev => ({ 
                              ...prev, 
                              assignedStaffIds: newStaffIds, 
                              assignedStaffNames: updatedNames,
                              assignedStaffName: updatedNames.length > 0 ? updatedNames[0] : null
                            }));
                            showToast('✓ Technicians updated.');
                          } catch (err) {
                            showToast(err.message || 'Failed to update assigned staff.', 'error');
                          }
                        }}
                        className="w-3.5 h-3.5 rounded border-white/20 bg-white/5 focus:ring-0 text-[#009bf2]"
                      />
            ...
            {/* Smart Suggestions / Nearby Jobs Widget Placeholder */}
            <div className="p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl space-y-2">
              <h4 className="text-indigo-400 font-bold text-[11px] flex items-center gap-1.5 uppercase tracking-wider">
                📍 Smart Suggestions (Radius Optimization)
              </h4>
              <div className="flex items-start gap-2">
                <span className="text-2xl">🤖</span>
                {loadingSuggestion ? (
                  <p className="text-[10px] text-indigo-300 flex items-center gap-1">
                    <Loader2 size={12} className="animate-spin" /> Calling Google Maps API for drive times...
                  </p>
                ) : suggestion ? (
                  <p className="text-[10px] text-indigo-300 leading-snug">
                    Based on Google Maps drive times, assigning to <span className="font-bold text-white bg-indigo-500/30 px-1 py-0.5 rounded">{suggestion.staff.name}</span> is recommended. 
                    They are currently <strong>{suggestion.driveTimeText}</strong> ({suggestion.distanceText}) away from this property.
                  </p>
                ) : (
                  <p className="text-[10px] text-indigo-300 leading-snug">
