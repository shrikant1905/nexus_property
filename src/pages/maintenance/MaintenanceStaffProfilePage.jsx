    ...
    <div className="space-y-6 max-w-4xl mx-auto text-slate-800">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          {staff?.avatarUrl ? (
            <img
              src={staff.avatarUrl.startsWith('/uploads') ? `https://nexusfmsbackednshrikant-production.up.railway.app${staff.avatarUrl}` : staff.avatarUrl}
              alt={staff.name}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
        <h2 className="text-base font-black text-[#00204a] flex items-center gap-2">
          <AlertCircle size={18} className="text-amber-500" /> Personal Performance & KPIs
        </h2>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
            <p className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mb-1">Total Points</p>
            <p className="text-2xl font-black text-[#00204a]">{staff?.kpiScore || 0}</p>
          </div>
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-center">
            <p className="text-[10px] font-extrabold text-emerald-800 uppercase tracking-wider mb-1">Jobs Completed</p>
            <p className="text-2xl font-black text-emerald-700">{staff?.jobsCompleted || 0}</p>
          </div>
          <div className="p-4 rounded-xl bg-red-50 border border-red-200 text-center">
            <p className="text-[10px] font-extrabold text-red-800 uppercase tracking-wider mb-1">Revisits</p>
            <p className="text-2xl font-black text-red-700">{staff?.revisits || 0}</p>
          </div>
          <div className="p-4 rounded-xl bg-sky-50 border border-sky-200 text-center flex flex-col justify-center items-center">
            <p className="text-[10px] font-extrabold text-sky-800 uppercase tracking-wider mb-1">Current Rank</p>
            <div className="flex items-center gap-1 text-lg font-black text-sky-700">
              #{staff?.currentRank || '-'} <ShieldCheck size={14} />
            </div>
          </div>
        </div>
      </div>
